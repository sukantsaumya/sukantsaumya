...
- **Description:** Engineered a full-stack quantitative trading platform to find, backtest, and analyze statistical arbitrage strategies. Features a modular Python backend and a dynamic, responsive Flask web application.
- **Tech Stack:** `Python` `Flask` `Pandas` `Statsmodels` `Plotly` `yfinance` `Requests` `HTML/CSS`
- **Features:**
    - Implements both **Engle-Granger** and **Johansen** cointegration tests for robust pair finding.
    - Deploys a professional web dashboard with a multi-market (S&P 500, FTSE 100, DAX) company screener.
    - Generates detailed, interactive HTML reports for each backtest, including metrics, trade logs, and charts.
